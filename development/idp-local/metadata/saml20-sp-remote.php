<?php
/**
 * SAML 2.0 remote SP metadata for SimpleSAMLphp.
 *
 * See: https://simplesamlphp.org/docs/stable/simplesamlphp-reference-sp-remote
 */

// Custom host with port number (for testing from browser on host O/S):
$metadata['http://profilereview-sp.local:52041'] = [
    'NameIDFormat' => 'urn:oasis:names:tc:SAML:2.0:nameid-format:persistent',
	'AssertionConsumerService' => 'http://profilereview-sp.local:52041/module.php/saml/sp/saml2-acs.php/profilereview-idp',
	'SingleLogoutService' => 'http://profilereview-sp.local:52041/module.php/saml/sp/saml2-logout.php/profilereview-idp',
    'certData' => '',
];

// Container name, no port number (for testing from another docker container):
$metadata['http://sp'] = [
    'NameIDFormat' => 'urn:oasis:names:tc:SAML:2.0:nameid-format:persistent',
	'AssertionConsumerService' => 'http://sp/module.php/saml/sp/saml2-acs.php/profilereview-idp-no-port',
	'SingleLogoutService' => 'http://sp/module.php/saml/sp/saml2-logout.php/profilereview-idp-no-port',
    'certData' => '',
];

// Copy both (modified) for use by local password manager SP.
$metadata['http://profilereview-pw-manager.local:52042'] = [
    'NameIDFormat' => 'urn:oasis:names:tc:SAML:2.0:nameid-format:persistent',
	'AssertionConsumerService' => 'http://profilereview-pw-manager.local:52042/module.php/saml/sp/saml2-acs.php/profilereview-idp',
	'SingleLogoutService' => 'http://profilereview-pw-manager.local:52042/module.php/saml/sp/saml2-logout.php/profilereview-idp',
    'certData' => '',
];
$metadata['http://pwmanager'] = [
    'NameIDFormat' => 'urn:oasis:names:tc:SAML:2.0:nameid-format:persistent',
	'AssertionConsumerService' => 'http://pwmanager/module.php/saml/sp/saml2-acs.php/profilereview-idp-no-port',
	'SingleLogoutService' => 'http://pwmanager/module.php/saml/sp/saml2-logout.php/profilereview-idp-no-port',
    'certData' => '',
];
