<?php
/**
 * SAML 2.0 remote SP metadata for SimpleSAMLphp.
 *
 * See: https://simplesamlphp.org/docs/stable/simplesamlphp-reference-sp-remote
 */

// Custom host with port number (for testing from browser on host O/S):
$metadata['http://profilereview-sp.local:8281'] = [
    'NameIDFormat' => 'urn:oasis:names:tc:SAML:2.0:nameid-format:persistent',
	'AssertionConsumerService' => 'http://profilereview-sp.local:8281/module.php/saml/sp/saml2-acs.php/profilereview-idp',
	'SingleLogoutService' => 'http://profilereview-sp.local:8281/module.php/saml/sp/saml2-logout.php/profilereview-idp',
    'certData' => '',
];

// Container name, no port number (for testing from another docker container):
$metadata['http://sp'] = [
    'NameIDFormat' => 'urn:oasis:names:tc:SAML:2.0:nameid-format:persistent',
	'AssertionConsumerService' => 'http://sp/module.php/saml/sp/saml2-acs.php/profilereviewidp-no-port',
	'SingleLogoutService' => 'http://sp/module.php/saml/sp/saml2-logout.php/profilereviewidp-no-port',
    'certData' => '',
];

// Copy both (modified) for use by local password manager SP.
$metadata['http://profilereviewpw-manager.local:8282'] = [
    'NameIDFormat' => 'urn:oasis:names:tc:SAML:2.0:nameid-format:persistent',
	'AssertionConsumerService' => 'http://profilereviewpw-manager.local:8282/module.php/saml/sp/saml2-acs.php/profilereviewidp',
	'SingleLogoutService' => 'http://profilereviewpw-manager.local:8282/module.php/saml/sp/saml2-logout.php/profilereviewidp',
    'certData' => '',
];
$metadata['http://pwmanager'] = [
    'NameIDFormat' => 'urn:oasis:names:tc:SAML:2.0:nameid-format:persistent',
	'AssertionConsumerService' => 'http://pwmanager/module.php/saml/sp/saml2-acs.php/profilereviewidp-no-port',
	'SingleLogoutService' => 'http://pwmanager/module.php/saml/sp/saml2-logout.php/profilereviewidp-no-port',
    'certData' => '',
];
